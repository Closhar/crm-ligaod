  ...
  <div class="modal-overlay" @mousedown.self="$emit('close')">
    <div class="modal-content" @click.stop>
      <div class="modal-header">
        <h2 class="modal-title">Редактирование видов спорта - {{ person?.last_name }} {{ person?.first_name }} {{ person?.middle_name || '' }}</h2>
      ...
      <div class="modal-body">
        ...
        <div v-if="editingMembership" class="mb-6">
          <h3 class="text-lg font-medium mb-4">Редактирование вида спорта</h3>
          <form @submit.prevent="saveMembership" class="space-y-4">
            <div class="form-row">
              <div class="form-group">
                <label class="form-label">Вид спорта</label>
                <KirhSelectField
                  v-model="editingMembership.sport_id"
        ...
        <div v-else class="mb-6">
          <h3 class="text-lg font-medium mb-4">Текущие виды спорта</h3>
          <div v-if="memberships.length > 0" class="space-y-3">
            <div v-for="membership in memberships" :key="membership.id" class="membership-item">
              <div class="flex justify-between items-center">
                <div>
                  <div class="font-medium">{{ membership.sport?.name }}</div>
          ...
          <div v-else class="text-gray-500">Виды спорта не указаны</div>
        </div>

        <!-- Форма добавления -->
        <div v-if="!editingMembership" class="border-t pt-6">
          <h3 class="text-lg font-medium mb-4">Добавить вид спорта</h3>
          <form @submit.prevent="addMembership" class="space-y-4">
            <div class="form-row">
              <div class="form-group">
                <label class="form-label">Вид спорта</label>
                <KirhSelectField
                  v-model="newMembership.sport_id"
                  api-url="/api/people/sports"
                  :api-params="{ type: 'async' }"
                  key-field="id"
                  label-field="name"
...
const loadMemberships = async () => {
  try {
    const response = await apiRequest(`/people/${props.person.id}/sport-memberships`)
    if (response.success) {
...
const addMembership = async () => {
  if (!newMembership.value.sport_id) return
  ...
  try {
    const response = await apiRequest(`/people/${props.person.id}/sport-memberships`, {
      method: 'POST',
      body: {
        sport_id: newMembership.value.sport_id,
...
const deleteMembership = async (membership) => {
  ...
  confirmationModal.value = {
    ...
    message: `Вы уверены, что хотите удалить вид спорта "${membership.sport?.name}"?`,
    onConfirm: async () => {
      try {
        const response = await apiRequest(`/people/${props.person.id}/sport-memberships/${membership.id}`, {
          method: 'DELETE'
        })
        ...
        console.error('Ошибка удаления вида спорта:', error)
      }
    }
  }
}
...
const saveMembership = async () => {
  if (!editingMembership.value.sport_id) return
  
  const requestData = {
    sport_id: editingMembership.value.sport_id,
  ...
  saving.value = true
  try {
    const response = await apiRequest(`/people/${props.person.id}/sport-memberships/${editingMembership.value.id}`, {
      method: 'PUT',
      body: requestData
    })
    ...
    console.error('Ошибка обновления вида спорта:', error)
  } finally {
    saving.value = false
  }
}
