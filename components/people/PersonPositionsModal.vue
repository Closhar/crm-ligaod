  ...
  <div class="modal-overlay" @mousedown.self="$emit('close')">
    <div class="modal-content" @click.stop>
      <div class="modal-header">
        <h2 class="modal-title">Редактирование должностей - {{ person?.last_name }} {{ person?.first_name }} {{ person?.middle_name || '' }}</h2>
      ...
      <div class="modal-body">
          ...
          <form @submit.prevent="saveMembership" class="space-y-4">
            <div class="form-row">
              <div class="form-group">
                ...
                <PositionSelect v-model="editingMembership.position_id" placeholder="Выберите или введите должность" required />
              </div>
        ...
        <div v-else class="mb-6">
          <h3 class="text-lg font-medium mb-4">Текущие должности</h3>
          <div v-if="memberships.length > 0" class="space-y-3">
            <div v-for="membership in memberships" :key="membership.id" class="membership-item">
              <div class="flex justify-between items-center">
                <div>
                  <div class="font-medium">{{ membership.position?.name }}</div>
          ...
          <div v-else class="text-gray-500">Должности не указаны</div>
        </div>
        ...
        <div v-if="!editingMembership" class="border-t pt-6">
          <form @submit.prevent="addMembership" class="space-y-4">
            <div class="form-row">
              <div class="form-group">
                <label class="form-label">Должность</label>
                <PositionSelect v-model="newMembership.position_id" placeholder="Выберите или введите должность" required />
              </div>
...
import PositionSelect from '~/components/people/PositionSelect.vue'
...
const loadMemberships = async () => {
  try {
    const response = await apiRequest(`/people/${props.person.id}/position-memberships`)
    if (response.success) {
...
const addMembership = async () => {
  if (!newMembership.value.position_id) return
  ...
  try {
    const response = await apiRequest(`/people/${props.person.id}/position-memberships`, {
      method: 'POST',
      body: {
        position_id: newMembership.value.position_id,
    ...
    if (response.success) {
      ...
      emit('saved') // Теперь родитель обновит основную таблицу
    }
  } catch (error) {
...
const deleteMembership = async (membership) => {
  ...
  confirmationModal.value = {
    ...
    message: `Вы уверены, что хотите удалить должность "${membership.position?.name}"?`,
    onConfirm: async () => {
      try {
        const response = await apiRequest(`/people/${props.person.id}/position-memberships/${membership.id}`, {
          method: 'DELETE'
        })
        if (response.success) {
          await loadMemberships()
...
const saveMembership = async () => {
  if (!editingMembership.value.position_id) return
  
  const requestData = {
    position_id: editingMembership.value.position_id,
  ...
  saving.value = true
  try {
    const response = await apiRequest(`/people/${props.person.id}/position-memberships/${editingMembership.value.id}`, {
      method: 'PUT',
      body: requestData
    })
...
const confirmAction = async () => {
  if (confirmationModal.value.onConfirm) {
    ...
    // emit('saved') // убираем, теперь обновление через emit('deleted')
  }
