<template>
  <div title="">
    <button
        ...
        @click="openModal"
    >
      <Icon :name="options.title_icon || 'ph:robot'" class="mx-auto mt-1" size="1.6em"/>
    </button>
      ...
      >
        <div class="p-4 border-b flex justify-between items-center">
          <h3 class="text-lg font-semibold">
            {{ options.title || 'AI Редактор текста' }}
          </h3>
          <div class="flex items-center gap-2">
            <button
                v-if="options.telegram_parse"
                title="Telegram Парсер"
            ...
            >
              <Icon name="ph:brain" size="18"/>
              <span class="text-sm font-medium">Парсинг и AI</span>
            </button>
            <button
        ...
        <div class="p-4 border-t flex justify-end space-x-2">
          ...
          <button
              ...
              :class="{ 'bg-orange-500 hover:bg-orange-600': hasChanges }"
              @click="saveChanges"
              :disabled="isSaving"
          >
            {{ isSaving ? 'Сохранение...' : (hasChanges ? 'Сохранить*' : 'Сохранить') }}
          </button>
        </div>
      </div>
    </div>

    <!-- Telegram Парсер модальное окно -->
    <TelegramParser
      v-if="showTelegramParser"
...
const openModal = () => {
  ...
  hasChanges.value = false; // Сбрасываем флаг изменений при открытии
...
const closeModal = () => {
  ...
  // Не отправляем событие blur при закрытии, чтобы избежать ненужных запросов
}
...
const cancelChanges = () => {
  ...
  // Восстанавливаем исходное значение без отправки на сервер
  localValue.value = getNormalizedValue(props.modelValue);
...
const handleValueUpdate = (newValue) => {
  // Только обновляем локальное значение, не отправляем на сервер
  localValue.value = getNormalizedValue(newValue);
  hasChanges.value = true; // Отмечаем что есть изменения
}

const saveChanges = () => {
  if (isSaving.value) return;
  
  // Отправляем изменения на сервер только при нажатии кнопки Сохранить
  isSaving.value = true;
  emit('update:modelValue', localValue.value);
  emit('blur', localValue.value); // Добавляем событие blur для сохранения в БД
  hasChanges.value = false; // Сбрасываем флаг изменений
  isSaving.value = false;
  closeModal();
}
